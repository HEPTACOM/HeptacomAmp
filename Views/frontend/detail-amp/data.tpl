{block name="frontend_detail-amp_data"}
	{if !$sArticle.liveshoppingData.valid_to_ts}
		...
		{if $sArticle.sBlockPrices && !$sArticle.liveshoppingData.valid_to_ts}
			...
			{block name="frontend_detail-amp_data_block_price_include"}
		...
		{else}
			<div class="sw-product--price sw-price--default{if $sArticle.has_pseudoprice} sw-price--discount{/if}">
				{* Default price *}
				{block name="frontend_detail-amp_data_price_configurator"}
					{if $sArticle.priceStartingFrom && !$sArticle.sConfigurator && $sView}
						{* Price - Starting from *}
						{block name="frontend_detail-amp_data_price_configurator_starting_from_content"}
					...
					{else}
						{* Regular price *}
						{block name="frontend_detail-amp_data_price_default"}
				...
				{* Discount price *}
				{block name="frontend_detail-amp_data_pseudo_price"}
					{if $sArticle.has_pseudoprice}
						{block name="frontend_detail-amp_data_pseudo_price_discount_icon"}
		...
		{if $sArticle.purchaseunit}
			{block name="frontend_detail-amp_data_price"}
				<div class="sw-product--price sw-price--unit">
					...
					{* Unit price content *}
					{block name="frontend_detail-amp_data_price_unit_content"}
					...
					{if $sArticle.purchaseunit && $sArticle.purchaseunit != $sArticle.referenceunit}
						...
						{block name="frontend_detail-amp_data_price_unit_reference_content"}
