{function name=categories level=0}
	{if $categories}
		<ul class="sw-sidebar--navigation sw-categories--navigation sw-navigation--list sw-is--level{$level}">
			{block name="frontend_HEPTACOMAMP_categories_left_before"}{/block}
			{foreach $categories as $category}
				{block name="frontend_HEPTACOMAMP_categories_left_entry"}
					<li class="sw-navigation--entry{if $category.flag} sw-is--active{/if}{if $category.subcategories} sw-has--sub-categories{/if}">
						...
						{block name="frontend_HEPTACOMAMP_categories_left_entry_subcategories"}
				...
				{/block}
			{/foreach}
			{block name="frontend_HEPTACOMAMP_categories_left_after"}{/block}
		</ul>
	{/if}
{/function}
