{block name="frontend_heptacom_amp_data"}
	<div class="sw-product--details">
		{if !$sArticle.liveshoppingData.valid_to_ts}
			...
			{if $sArticle.sBlockPrices && !$sArticle.liveshoppingData.valid_to_ts}
				...
				{block name="frontend_heptacom_amp_data_block_price_include"}
			...
			{else}
				<div class="sw-product--price sw-price--default{if $sArticle.has_pseudoprice} sw-price--discount{/if}">
					{* Default price *}
					{block name="frontend_heptacom_amp_data_price_configurator"}
						{if $sArticle.priceStartingFrom && !$sArticle.sConfigurator && $sView}
							{* Price - Starting from *}
							{block name="frontend_heptacom_amp_data_price_configurator_starting_from_content"}
						...
						{else}
							{* Regular price *}
							{block name="frontend_heptacom_amp_data_price_default"}
					...
					{* Discount price *}
					{block name="frontend_heptacom_amp_data_pseudo_price"}
						{if $sArticle.has_pseudoprice}
							{block name="frontend_heptacom_amp_data_pseudo_price_discount_icon"}
							...
							{block name="frontend_heptacom_amp_data_pseudo_price_discount_content"}
								<span class="sw-content--discount">
									{block name="frontend_heptacom_amp_data_pseudo_price_discount_before"}
									...
									{block name="frontend_heptacom_amp_data_pseudo_price_discount_after"}
										{s name="priceDiscountInfo" namespace="frontend/detail/data"}{/s}
									{/block}

									{* Percentage discount *}
									{block name="frontend_heptacom_amp_data_pseudo_price_discount_content_percentage"}
			...
			{if $sArticle.purchaseunit}
				{block name="frontend_heptacom_amp_data_price"}
					<div class="sw-product--price sw-price--unit">

						{* Unit price label *}
						{block name="frontend_heptacom_amp_data_price_unit_label"}
						...
						{* Unit price content *}
						{block name="frontend_heptacom_amp_data_price_unit_content"}
						...
						{if $sArticle.purchaseunit && $sArticle.purchaseunit != $sArticle.referenceunit}
							...
							{block name="frontend_heptacom_amp_data_price_unit_reference_content"}
			...
			{* Tax information *}
			{block name="frontend_heptacom_amp_data_tax"}
...
{if $sOutputNet}zzgl.{else}inkl.{/if} MwSt. <a title="Versandkosten" href="/versand-und-zahlung">zzgl. Versandkosten</a>
		...
		{block name="frontend_heptacom_amp_supplier_info"}
		...
		{block name="frontend_heptacom_amp_data_delivery"}
